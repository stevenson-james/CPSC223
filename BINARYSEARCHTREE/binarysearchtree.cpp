...
// pre: treep is assigned a pointer to a root node of a binary search tree, targetText is 
//       a Key and count is assigned to numberOfItems
// post: if targetText is not in binary search tree, then exception is thrown
//       else the targetText that corresponds to the Item is deleted
// usage: deleteHelper(root, targetText, numberOfItems);
